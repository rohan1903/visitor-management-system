...
#             "time_spent": None,
#             "status": "Registered",
#             "visit_approved": True if not employee_id else False,
#             "has_visited": False
#         }

#         # Save the visit under visitor
#         db_ref.child(f"visitors/{visitor_id}/visits/{visit_id}").set(visit_data)
#         logger.info(f"✅ NEW VISIT CREATED under visitor {visitor_id}")
...
#         if employee_id and employee_data and employee_data.get('email'):
#             employee_email = employee_data.get('email')
#             profile_url = f"https://174bdd68796e.ngrok-free.app/employee_action/{visitor_id}"
...
# @app.route("/employee_action_confirm/<visitor_id>", methods=["POST"])
# def employee_action_confirm(visitor_id):
#     try:
#         data = request.get_json()
#         if not data or data.get("action") != "approve":
...
#             if not vdata.get("has_visited", False) and vdata.get("status") == "Registered":
#                 latest_visit_id = vid
#                 break

#         if not latest_visit_id:
#             return jsonify({"status": "error", "message": "No pending visit found"}), 404
...
#             "employee_action": "approve",
#             "action_taken_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
#         })

#         logger.info(f"Visitor {visitor_id} visit {latest_visit_id} approved by employee")
#         return jsonify({"status": "success", "message": "Visitor approved successfully"})
...
#             "status": "Rejected",
#             "employee_action_taken": True,
#             "employee_action": "reject",
#             "rejection_reason": rejection_reason,
#             "action_taken_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
#         })

#         logger.info(f"Visitor {visitor_id} visit {latest_visit_id} rejected by employee")
#         return jsonify({"status": "success", "message": "Visitor rejected successfully"})
...
#         reschedule_reason = data.get("reschedule_reason", "No reason provided")
        
#         if action != "reschedule" or not new_visit_date:
#             return jsonify({"status": "error", "message": "Invalid action or missing date"}), 400
...
#             "status": "Rescheduled",
#             "employee_action_taken": True,
#             "employee_action": "reschedule",
#             "new_visit_date": new_visit_date,
#             "reschedule_reason": reschedule_reason,
#             "action_taken_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
#         })

#         logger.info(f"Visitor {visitor_id} visit {latest_visit_id} rescheduled to {new_visit_date}")
#         return jsonify({"status": "success", "message": f"Visit rescheduled to {new_visit_date}"})

#     except Exception as e:
#         logger.error(f"Error rescheduling visitor {visitor_id}: {e}")
#         return jsonify({"status": "error", "message": "Something went wrong"}), 500
...
# Initialize Firebase (make sure you have this setup)
...
#         # Get the latest visit ID from visitor data
#         latest_visit_id = visitor_data.get('visit_id')
        
#         if not latest_visit_id:
#             return "No visits found for this visitor", 404
        
#         # Get the specific visit data
#         visit_ref = db.reference(f'visitors/{visitor_id}/visits/{latest_visit_id}')
#         visit_data = visit_ref.get()
        
#         if not visit_data:
#             return "Visit data not found", 404
...
#                              visit_date=visit_data.get('visit_date', 'Not specified'),
#                              photo_url=visitor_data.get('photo_url'))
#     except Exception as e:
